...
if `fromno' < 570 {
  display as error "You specified from(`from'), which is before records began." _newline "Option from() must be later th
> an 2007m7."
  exit
}
if `tono' < 570 {
  display as error "You specified to(`to'), which is before records began." _newline "Option to() must be later than 200
> 7m7."
  exit
}
if `fromno' > `tono' {
...
if "`graph'" != "" & "`author'" == "" & "`package'" == "" {
  display as error "No authors or packages have been selected, but the graph option has." _newline "The thousands of sma
> ll graphs will not be drawn."
}
else if `"`graph'"' != "" {
  quietly tab `command' author
  if `r(r)'==1 & `r(c)'==1 { 
    twoway (line npkghit mo) (lowess npkghit mo) , ytit("Number of hits") ylab(,format(%9.0f)) xlab(,angle(45)) ylab(,an
> gle(0))
  }
  else if `r(r)'==1 & `r(c)'>1 {
    noisily twoway (line npkghit mo) (lowess npkghit mo) , by(author,note("")) ytit("Number of hits") ylab(,format(%9.0f
> )) xlab(,angle(45)) ylab(,angle(0))
  }
  else if `r(r)'>1 & `r(c)'==1 {
    noisily twoway (line npkghit mo) (lowess npkghit mo) , by(`command',note("")) ytit("Number of hits") ylab(,format(%9
> .0f)) xlab(,angle(45)) ylab(,angle(0))
  }
  else if `r(r)'>1 & `r(c)'>1 {
    noisily twoway (line npkghit mo) (lowess npkghit mo) , by(author `command',note("")) ytit("Number of hits") ylab(,fo
> rmat(%9.0f)) xlab(,angle(45)) ylab(,angle(0))
  }
}
